    ...
    <div class="container">
        ...
        <div v-else class="container bd-gutter bd-layout margin">
            <article
                ...
                v-if="page"
                :class="{ full: page.rightBar === false }"
            >
...
if (slug === "/blogs/" || slug === "/blogs/community") {
    if (slug === "/blogs/") {
        const { data: pageData } = await useAsyncData(`Blog-Page-List`, () =>
    ...
    const { data: externalNewsData } = await useAsyncData(
        `blog-external-news`,
        () => {
            return $fetch(
                `https://api.kestra.io/v1/external-blogs${
                    slug === "/blogs/" ? "?size=4" : ""
                }`
            );
        }
